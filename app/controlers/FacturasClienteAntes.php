...
class FacturasCliente extends Controlador {
    ...
    {                       
        ...
        if(isset($_POST['idcliente']) && $_POST['idcliente'] > 0 && isset($_POST['estado']) && $_POST['estado'] !='' && trim($_POST['cif']) !='' && $_POST['fecha'] != '' ){                              
                ...
                $numeracion = DocumentHelper::buildNumberDocument($this->modeloBase->maximoNumDocumentoAnio('numerointerno',$this->tabla, date("Y",strtotime($_POST['fecha']))), $_POST['fecha']);
    ...
    private function crearCabeceraFactura($post)
    {
        ...
        $numeracion = DocumentHelper::buildNumberDocument($this->modeloBase->maximoNumDocumentoAnio('numerointerno',$this->tabla, date("Y",strtotime(date("Y-m-d")))), date("Y-m-d"));
    ...
    public function eliminarFactura() 
    {
        ...
        if($_POST['id'] > 0){
            ...
            if($facturaActual) {
                ...
                $existenPosteriores = $this->ModelFacturasCliente->verificarSiExistenFacturasPosteriores($_POST['id'], $anioFactura, $facturaActual->numerointerno);              
