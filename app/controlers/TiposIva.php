...
class TiposIva extends Controlador {
    ...
    private function mapearCampoOrdenTipoIva($campoVisible) {
        $mapa = [
            'Nº'            => 'id',
            'Tipo IVA (%)'  => 'tipoiva',
            'estado'        => 'activo'
    ...
    private function construirClausulaOrderByTiposIva($ordenMultipleJson, $ordenSimple, $tipoSimple) {
        ...
        if (!empty($ordenMultipleJson)) {
            ...
            if (is_array($ordenes) && count($ordenes) > 0) {
                ...
                foreach ($ordenes as $item) {
                    ...
                    $campoSQL = $this->mapearCampoOrdenTipoIva($campoVisible);
    ...
    public function crearTablaTiposIva()
    {
        ...
        if (!empty($ordenMultiple)) {
            ...
            $clausulaOrder = $this->construirClausulaOrderByTiposIva($ordenMultiple, $ordenSimple, $tipoSimple);
        ...
        $tiposIva = $this->ModelTiposIva->obtenerTiposIvaTablaClassBuscar($filas,$ordenFinal,$filaspagina,$tipoFinal,$cond);
